...
void SCCB_Init(void);
/*-------------OV2640-----------------*/
uint8_t OV2640_Init(void);//初始化OV2640，默认初始化后输出1600*1200图片
void OV2640_JPEG_Mode(void);//OV2640切换JPEG模式
void OV2640_RGB565_Mode(void);//OV2640切换REB565模式
void OV2640_Auto_Exposure(unsigned char level);//OV2640自动曝光等级设置0~4
void OV2640_Light_Mode(unsigned char mode);//白平衡设置0~4
void OV2640_Color_Saturation(unsigned char sat);//色度设置0~4
void OV2640_Brightness(unsigned char bright);//亮度设置0~4
void OV2640_Contrast(unsigned char contrast);//对比度设置0~4
void OV2640_Special_Effects(unsigned char eft);//特效设置0~6
void OV2640_Color_Bar(unsigned char sw);//彩条测试，1开启，0关闭
void OV2640_Set_JPEG_Quality(unsigned char quality);//JPEG画质设置1~10
...
//定义OV2640的PWDN和RESET即PG9和PG15的相关操作
...
#define SCCB_SDA_READ()  HAL_GPIO_ReadPin(DCMI_SDA_GPIO_Port,DCMI_SDA_Pin)//输入SDA    
